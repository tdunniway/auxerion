...
**Date:** 2026-09-14 · **Pages:** /creativ8, /moose, /kept · **Status:** implemented
...
| Kept | https://foodopsapp.vercel.app | demo session, no login. The demo control bar is removed per screen: cropped by measured height on desktop, hidden on phone; tablet presets render inside an in-app bezel, so those clip to the screen element. |

## Screen selection

Chosen for being populated, legible at card size, and each showing a different job the product does. Empty or test-data views (MOOSE notes and handoffs, Creativ8 levels/metagame/character page, Kept worker tablet under the arrival workflow) were rejected after review.
...
- **Kept (5 + 2 + device set):** hotel today, front desk station, arrival plan, restaurant POS, guest website; a two-tablet row (manager live property, guest tablet); the existing device set refreshed with hotel today (PC), crew tablet, guest phone "My Stay".
